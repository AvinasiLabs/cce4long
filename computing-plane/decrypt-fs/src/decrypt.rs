...
pub fn encrypt_avin(dek: &Key, plaintext: &[u8]) -> Vec<u8> {
...
mod tests {
    ...
    fn roundtrip_single_chunk() {
        ...
        let encrypted = encrypt_avin(&key, plaintext);
    ...
    fn roundtrip_empty() {
        let key = test_key();
        let encrypted = encrypt_avin(&key, b"");
        let decrypted = decrypt_avin(&key, &encrypted).unwrap();
    ...
    fn bad_magic_rejected() {
        let key = test_key();
        let mut data = encrypt_avin(&key, b"test");
    ...
    fn wrong_dek_fails() {
        ...
        let encrypted = encrypt_avin(&key1, b"secret data");
    ...
    fn truncated_data_fails() {
        let key = test_key();
        let encrypted = encrypt_avin(&key, b"test");
